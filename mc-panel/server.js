...
// ── Forge Versions ────────────────────────────────────────────
app.get('/api/forge/versions', authMw, async (_req, res) => {
  try {
    const data = await fetchJson('https://files.minecraftforge.net/net/minecraftforge/forge/promotions_slim.json');
    // extract unique MC versions that have a recommended or latest build, newest first
    const seen = new Set();
    const versions = [];
    for (const key of Object.keys(data.promos).reverse()) {
      const mcVer = key.replace(/-(recommended|latest)$/, '');
      if (!seen.has(mcVer)) { seen.add(mcVer); versions.push(mcVer); }
    }
    res.json(versions);
  } catch(e) { res.status(500).json({ error: String(e) }); }
});
...
async function buildServer(config, log) {
  ...
  if (type === 'forge')  return buildForge(config, log);
...
// ── Forge Builder ──────────────────────────────────────────────
async function buildForge(config, log) {
  let { version, name, port, ram, maxPlayers, difficulty, gamemode, onlineMode, motd } = config;
  name = (name || 'minecraft-server').toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/^-+|-+$/g, '');
  ...
  log(`► Suche Forge Build für MC ${version}...`);
  const promos = await fetchJson('https://files.minecraftforge.net/net/minecraftforge/forge/promotions_slim.json');
  const forgeVer = promos.promos[`${version}-recommended`] || promos.promos[`${version}-latest`];
  if (!forgeVer) throw new Error(`Kein Forge Build für MC ${version} gefunden`);
  const fullVer    = `${version}-${forgeVer}`;
  const installerUrl = `https://maven.minecraftforge.net/net/minecraftforge/forge/${fullVer}/forge-${fullVer}-installer.jar`;
  log(`  Forge ${fullVer} gefunden.`);

  log(`► Erstelle Verzeichnisse...`);
  fs.mkdirSync(dataDir, { recursive: true });

  log(`► Lade Forge Installer herunter...`);
  const installerPath = `${dataDir}/forge-installer.jar`;
  await downloadFile(installerUrl, installerPath, pct => log(`  Download ${pct}%`, 'progress'));

  fs.writeFileSync(`${dataDir}/eula.txt`, 'eula=true\n');
  try { fs.chownSync(dataDir, 1001, 1001); } catch {}

  const rconPass   = crypto.randomBytes(12).toString('hex');
  const serverMotd = motd || `${name} | Forge ${fullVer}`;
  writeServerFiles(serverDir, dataDir, name, port, rconPort, rconPass, ram, serverMotd, maxPlayers, difficulty, gamemode, onlineMode, version, 'forge', fullVer);
  // Store forge version in panel.json
  try {
    const pj = JSON.parse(fs.readFileSync(`${serverDir}/panel.json`,'utf8'));
    pj.forgeVersion = fullVer;
    fs.writeFileSync(`${serverDir}/panel.json`, JSON.stringify(pj, null, 2));
  } catch {}

  const jvm = jvmFlags(ram);
  // Dockerfile: JDK for installer, then run server via run.sh
  fs.writeFileSync(`${serverDir}/Dockerfile`,
`FROM eclipse-temurin:21-jdk-jammy AS installer
WORKDIR /server
COPY data/forge-installer.jar /server/forge-installer.jar
COPY data/eula.txt /server/eula.txt
RUN java -jar /server/forge-installer.jar --installServer 2>&1 | tail -5
...
STOPSIGNAL SIGTERM
USER minecraft
CMD ["sh", "-c", "if [ -f /server/run.sh ]; then exec sh /server/run.sh --nogui; else exec java $JVM_OPTS @libraries/net/minecraftforge/forge/${fullVer}/unix_args.txt nogui; fi"]
`);
  writeDockerCompose(serverDir, name, port, rconPort);
  log(`► Baue Docker Image (Forge Installer läuft – das dauert 2-5 Minuten)...`);
  await runDockerBuild(serverDir, name, port, log);
  log(`✔ Forge Server "${name}" läuft auf Port ${port}!`, 'success');
}
