...
// ── Velocity Versions ────────────────────────────────────────
app.get('/api/velocity/versions', authMw, async (_req, res) => {
  try {
...
async function buildServer(config, log) {
  ...
  if (type === 'custom')   return buildCustom(config, log);
  if (type === 'velocity') return buildVelocity(config, log);
...
// ── Custom JAR Upload ─────────────────────────────────────────
const customJarStore = new Map(); // jarId → { path, originalName, expires }

const jarUpload = multer({
  storage: multer.diskStorage({
    destination: (_req, _file, cb) => {
      const tmp = '/tmp/skitaru-jars';
      fs.mkdirSync(tmp, { recursive: true });
      cb(null, tmp);
    },
    filename: (_req, _file, cb) => cb(null, crypto.randomBytes(16).toString('hex') + '.jar'),
  }),
  limits: { fileSize: 500 * 1024 * 1024 },
  fileFilter: (_req, file, cb) => {
    if (!file.originalname.endsWith('.jar')) return cb(new Error('Nur .jar Dateien erlaubt'));
    const lname = file.originalname.toLowerCase();
    if (lname.includes('installer') || (lname.includes('forge') && !lname.includes('universal')))
      return cb(new Error('Forge Installer JARs werden nicht unterstützt. Bitte einen fertigen Server-JAR hochladen.'));
    cb(null, true);
  },
});

app.post('/api/upload-jar', authMw, (req, res) => {
  jarUpload.single('jar')(req, res, err => {
    if (err) return res.status(400).json({ error: err.message });
    if (!req.file) return res.status(400).json({ error: 'Keine Datei' });
    const jarId = crypto.randomBytes(16).toString('hex');
    customJarStore.set(jarId, {
      path: req.file.path,
      originalName: req.file.originalname,
      expires: Date.now() + 30 * 60 * 1000, // 30min
    });
    res.json({ jarId, name: req.file.originalname, size: req.file.size });
  });
});

// ── Custom Server Builder ─────────────────────────────────────
async function buildCustom(config, log) {
  let { name, port, ram, maxPlayers, difficulty, gamemode, onlineMode, motd, jarId, javaVersion, startArgs } = config;
  name = (name || 'custom-server').toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/^-+|-+$/g, '');

  const jarInfo = customJarStore.get(jarId);
  if (!jarInfo) throw new Error('JAR nicht gefunden oder abgelaufen – bitte erneut hochladen');
  customJarStore.delete(jarId);
  ...
  log(`► Kopiere ${jarInfo.originalName} → server.jar...`);
  fs.copyFileSync(jarInfo.path, `${dataDir}/server.jar`);
  try { fs.unlinkSync(jarInfo.path); } catch {}
  try { fs.chownSync(dataDir, 1001, 1001); fs.chownSync(`${dataDir}/server.jar`, 1001, 1001); } catch {}

  fs.writeFileSync(`${dataDir}/eula.txt`, 'eula=true\n');
  try { fs.chownSync(`${dataDir}/eula.txt`, 1001, 1001); } catch {}

  const rconPass   = crypto.randomBytes(12).toString('hex');
  const serverMotd = motd || `${name} | Custom`;
  const javaTag    = javaVersion || '21';
  const jvm        = jvmFlags(ram);
  const extra      = (startArgs || '').trim();
  const cmd        = extra ? `exec java $JVM_OPTS ${extra} -jar /server/server.jar nogui`
                           : `exec java $JVM_OPTS -jar /server/server.jar nogui`;

  writeServerFiles(serverDir, dataDir, name, port, rconPort, rconPass, ram, serverMotd, maxPlayers, difficulty, gamemode, onlineMode, 'custom', 'custom');
  try {
    const pj = JSON.parse(fs.readFileSync(`${serverDir}/panel.json`, 'utf8'));
    pj.jarName = jarInfo.originalName;
    pj.javaVersion = javaTag;
    fs.writeFileSync(`${serverDir}/panel.json`, JSON.stringify(pj, null, 2));
  } catch {}
...
`);
  writeDockerCompose(serverDir, name, port, rconPort);
  await runDockerBuild(serverDir, name, port, log);
  log(`✔ Custom Server "${name}" läuft auf Port ${port}!`, 'success');
}

// ── Velocity Builder ───────────────────────────────────────────
async function buildVelocity(config, log) {
  let { version, name, port, ram, motd } = config;
  name = (name || 'velocity-proxy').toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/^-+|-+$/g, '');

  const requestedPort = parseInt(port) || 25577;
  log(`► Prüfe Port ${requestedPort}...`);
  port = await findFreePort(requestedPort);
  if (port !== requestedPort) log(`  Port ${requestedPort} belegt → verwende Port ${port}.`);
  else log(`  Port ${port} ist frei.`);
  ...
  log(`► Suche Build für Velocity ${version}...`);
  const buildData   = await fetchJson(`https://api.papermc.io/v2/projects/velocity/versions/${version}/builds`);
  const builds      = buildData.builds;
  if (!builds?.length) throw new Error(`Kein Build für Velocity ${version}`);
  const latestBuild = builds[builds.length - 1].build;
  const jarName     = `velocity-${version}-${latestBuild}.jar`;
  const dlUrl       = `https://api.papermc.io/v2/projects/velocity/versions/${version}/builds/${latestBuild}/downloads/${jarName}`;
  log(`  Build ${latestBuild} gefunden.`);

  log(`► Erstelle Verzeichnisse...`);
  fs.mkdirSync(dataDir, { recursive: true });

  log(`► Lade velocity.jar herunter...`);
  const jarPath = `${dataDir}/velocity.jar`;
  await downloadFile(dlUrl, jarPath, pct => log(`  Download ${pct}%`, 'progress'));
  try { fs.chownSync(dataDir, 1001, 1001); fs.chownSync(jarPath, 1001, 1001); } catch {}
  ...
  // Parse backend servers from config (format: "name=host:port", one per line)
  const backendLines = (config.backendServers || '').trim().split('\n').map(l => l.trim()).filter(Boolean);
  const serverEntries = [];
  const serverNames   = [];
  for (const line of backendLines) {
    const m = line.match(/^([a-zA-Z0-9_-]+)\s*=\s*(.+)$/);
    if (m) { serverEntries.push(`${m[1]} = "${m[2]}"`); serverNames.push(m[1]); }
  }
  // Always ensure at least one server entry so Velocity can start
  if (serverEntries.length === 0) {
    serverEntries.push('lobby = "127.0.0.1:25565"');
    serverNames.push('lobby');
  }
  const tryList = serverNames.map(n => `"${n}"`).join(', ');

  // Generate forwarding secret
  const forwardingSecret = crypto.randomBytes(16).toString('hex');
  fs.writeFileSync(`${dataDir}/forwarding.secret`, forwardingSecret);
  try { fs.chownSync(`${dataDir}/forwarding.secret`, 1001, 1001); } catch {}

  const vMotd = (motd || name + ' | Velocity ' + version).replace(/"/g, '');
  fs.writeFileSync(`${dataDir}/velocity.toml`,
`# Velocity config – generated by Skitaru Panel
config-version = "2.7"
bind = "0.0.0.0:${port}"
motd = "${vMotd}"
...
port = ${port}
`);
  try { fs.chownSync(`${dataDir}/velocity.toml`, 1001, 1001); } catch {}

  // Store panel info
  fs.mkdirSync(serverDir, { recursive: true });
  fs.writeFileSync(`${serverDir}/panel.json`, JSON.stringify({
    name, serverType: 'velocity', version, build: latestBuild,
    port, rconPort, rconPass, javaVersion: '17',
    created: new Date().toISOString()
  }, null, 2));

  const jvm = `-Xms256M -Xmx${ram} -XX:+UseG1GC -XX:G1HeapRegionSize=4M -XX:+UnlockExperimentalVMOptions -XX:+ParallelRefProcEnabled -XX:+AlwaysPreTouch`;
...
STOPSIGNAL SIGTERM
USER minecraft
CMD ["sh", "-c", "exec java $JVM_OPTS -jar /server/velocity.jar"]
`);
...
`);
  ...
  await runDockerBuild(serverDir, name, port, log);
  log(`✔ Velocity Proxy "${name}" läuft auf Port ${port}!`, 'success');
}
