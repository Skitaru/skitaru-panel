...
step() {
  ...
  echo -e "  ${G}[${bar}]${N} ${DIM}${pct}%${N}  ${W}${n}/${total}${N}  ${label}"
  echo
}
...
run "curl, gnupg, ca-certificates" apt-get install -y -qq curl gnupg ca-certificates lsb-release apt-transport-https
...
if command -v node &>/dev/null && node -e 'process.exit(parseInt(process.version.slice(1))<18?1:0)' 2>/dev/null; then
  ...
  run "NodeSource GPG + repo" bash -c 'curl -fsSL https://deb.nodesource.com/setup_20.x | bash - >> /tmp/skitaru-install.log 2>&1'
...
run "server.js"      curl -fsSL "$REPO/server.js"         -o "$PANEL_DIR/server.js"
run "index.html"     curl -fsSL "$REPO/public/index.html" -o "$PANEL_DIR/public/index.html"
run "package.json"   curl -fsSL "$REPO/package.json"      -o "$PANEL_DIR/package.json"
