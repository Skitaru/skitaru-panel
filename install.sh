...
_dl() { curl -fsSL "$1" -o "$2" 2>/dev/null || wget -q "$1" -O "$2"; }
run "server.js"      bash -c '_dl "$REPO/server.js" "$PANEL_DIR/server.js"'
run "index.html"     bash -c '_dl "$REPO/public/index.html" "$PANEL_DIR/public/index.html"'
run "package.json"   bash -c '_dl "$REPO/package.json" "$PANEL_DIR/package.json"' 
